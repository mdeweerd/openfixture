...
def pcb_template(outline, pins, annular=0.25, xoff=0, yoff=0, hole_dia=None):
    ...
    for i, pin in enumerate(pins):
        ...
        net_defs.append(
            '(net {ip1} "{netname}")'.format(ip1=i + 1, netname=netname)
        )
        net_class_defs.append('(add_net "{netname}")'.format(netname=netname))

    edge_cuts = [
        '(gr_line (start {x1} {y1}) (end {x2} {y2}) (layer "Edge.Cuts") (width 0.05))'.format(
            x1=x1 + xoff, y1=y1 + yoff, x2=x2 + xoff, y2=y2 + yoff
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "outline", metavar="outline.dxf", help="Board outline DXF"
    )
    parser.add_argument(
        "output",
        default="kicad",
        help="Output directory/project name and path",
    )
    parser.add_argument(
        "--testpins",
        "-t",
        metavar="TESTPINS",
        default="DEMO1:20:20:NETNAME1,DEMO2:60:80:NETNAME2",
        help="List of test pins NAME1:X1:Y1:NETNAME1:diam1OrModel?,NAME2:X2:Y2:NETNAME2:diam1OrModel?",
    )
    parser.add_argument(
        "--xspace",
        "-x",
        type=int,
        default=1500,
        help="Schematic pin X spacing in mil (default: 1000)",
    )
    parser.add_argument(
        "--yspace",
        "-y",
        type=int,
        default=200,
        help="Schematic pin Y spacing in mil (default: 200)",
    )
    parser.add_argument(
        "--annular",
        "-a",
        type=float,
        default=0.25,
        help="Pogo pin annular ring width in mm (default: 0.25)",
    )
    parser.add_argument(
        "--diameter",
        "-d",
        ...
        help="Pogo pin diameter in mm (default: %f)" % (DEFAULT_HOLE_DIA),
    )
    parser.add_argument(
        "--name", "-n", default="jig", help="Output KiCAD project name"
    )
    ...
    if not path.isdir(args.output):
        raise SystemError(
            strf(F('Output path "{args.output}" is not a directory'))
        )
    ...
    with open(sch_name, "w", encoding="utf8") as sch:
        title = "{name} generated schematic (PogoJig v{version})".format(
            name=name, version=__version__
        )
        sch.write(
            sch_template(
                title, pins, xspace=args.xspace, yspace=args.yspace
            ).decode("utf8")
        )

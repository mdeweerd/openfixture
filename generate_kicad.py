    ...
    for i in range(num_pins):
        ...
        components_list.append(dedent('''
            ...
            '''.format(
                   i=i,
                   base=PIN_TS_BASE + i,
                   identifier=identifier.format(i=i),
                   value=value,
                   x=x,y=y,
...
def pcb_template(outline, pins, annular=0.5):
    ...
    for i, pin in enumerate(pins):
        ...
        mod = '''
          (module "Pogopin:AutogeneratedPogopinFootprint" (layer "F.Cu") (tedit {teditb:08X}) (tstamp {pintsb:08X})
            (at {x} {y})
            (descr "Pogo pin {i}")
            (tags "test point plated hole")
            (path "/{pathb:08X}")
            (attr virtual)
            (fp_text reference "TP{i}" (at 0 -{pad_radp1}) (layer "F.SilkS")
            ...
            (pad "1" thru_hole circle (at 0 0) (size {pad_dia} {pad_dia}) (drill {hole_dia}) (layers *.Cu *.Mask)
              (net {ip1} "pogo{i}"))
          )'''.format(i=i,ip1=i+1,pad_dia=pad_dia,hole_dia=hole_dia,x=x,y=y,teditb=TEDIT_BASE+i,pathb=PATH_BASE+i,pintsb=PIN_TS_BASE+i,pad_radp1=pad_dia/2+1)
    ...
    net_defs = [ '(net {ip1} "pogo{i}")'.format(i=i,ip1=i+1) for i, _pin in enumerate(pins) ]
    net_class_defs = [ '(add_net "pogo{i}")'.format(i=i) for i, _pin in enumerate(pins) ]
...
if __name__ == '__main__':
    ...
    #parser.add_argument('testpins', metavar='TESTPINS', help='List of test pins NAME1:X1:Y1,NAME2:X2:Y2')
    ...
    with open(sch_name, 'w', encoding='utf8') as sch:
        ...
        sch.write(sch_template(title, len(pins), yspace=args.yspace).decode('utf8'))

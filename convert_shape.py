...
import re
freecadPattern = re.compile('FreeCAD(Cmd)?(\.exe)?$', re.IGNORECASE)
freecadCmdPattern = re.compile('freecadcmd(\.exe)?$', re.IGNORECASE)
if freecadPattern.search(sys.argv[0]):
  ...
  paths = [ '/usr/lib64/freecad/lib', 'C:/Program Files/FreeCAD 0.20/bin', 'C:/Program Files/FreeCAD 0.19/bin', 'C:/Program Files/FreeCAD 0.18/bin' ]
  for p in paths:
    if(os.path.exists(p)):
      FREECADPATH = p
      break
  ...
  sys.exit(1)
    ...
    if type == "step":
      ...
      if hasGui:
        ...
        try:
          import ImportGui
          ImportGui.export(allObjects, name=oname, keepPlacement=True) #, legacy=True, keepPlacement=True, exportHidden=True)
          done = True
      ...
      if not done:
          try:
            import Import
            Import.export(obj=[data], name=oname, legacy=True, keepPlacement=True, exportHidden=True)
            done = True
          except Exception as e:
            print("export using Import did not work", e)

    elif type == "iges": data.Shape.exportIges(oname)
    elif type == "dae":
      import importDAE
      # mesh.write(Filename=oname)
      importDAE.export(data, oname)  # .write(Filename=oname)
    elif type == "wrl": mesh.write(Filename=oname)
    sys.exit(0)
